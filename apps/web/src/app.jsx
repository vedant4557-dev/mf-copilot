...
// ═══════════════════════════════════════════════════════════════
// PORTFOLIO OPTIMIZER — Modern Portfolio Theory + Scoring
// ═══════════════════════════════════════════════════════════════
function PortfolioOptimizerPage({an,holdings}){
  const [optimizing,setOptimizing]=useState(false);
  const [result,setResult]=useState(null);
  const [objective,setObjective]=useState("sharpe"); // sharpe | min_vol | max_return
  const [constraints,setConstraints]=useState({maxER:1.5,minFunds:3,maxFunds:8,maxSingle:40,noSmallCapAbove:30});
  const [aiRationale,setAiRationale]=useState("");

  // MPT-inspired scoring function
  const scoreAllocation=(funds)=>{
    const n=funds.length;
    if(n===0) return 0;
    const wER   = funds.reduce((s,f)=>(s+f.er*f.w),0)/100;
    const wRet  = funds.reduce((s,f)=>(s+(f.r3||0)*f.w),0)/100;
    const wVol  = funds.reduce((s,f)=>(s+f.vol*f.w),0)/100;
    const wSh   = funds.reduce((s,f)=>(s+(f.sharpe||0)*f.w),0)/100;
    // Category diversity bonus
    const cats  = new Set(funds.map(f=>f.cat));
    const divBonus = cats.size*5;
    // Overlap penalty (same AMC)
    const amcs  = funds.map(f=>f.amc);
    const dupPenalty = amcs.filter((a,i)=>amcs.indexOf(a)!==i).length*8;
    const score = (wRet/2) + (wSh*10) - (wER*15) - (wVol/4) + divBonus - dupPenalty;
    return {score:Math.round(score*10)/10, wER, wRet, wVol, wSh};
  };
  ...
  const runOptimizer=async()=>{
    ...
    // Build candidate pool — all funds sorted by objective
    const pool=Object.entries(FUNDS).map(([name,f])=>({name,...f}));
    
    const scored=pool.map(f=>{
      const raw = objective==="sharpe"   ? (f.sharpe||0) - f.er*2 :
                  objective==="min_vol"  ? -f.vol - f.er :
                                           (f.r3||0) - f.er;
      return {...f,raw};
    ...
    for(const f of scored){
      if(selected.length>=constraints.maxFunds) break;
      if(f.er>constraints.maxER) continue;
      if(f.cat==="Small Cap"&&remaining>100-constraints.noSmallCapAbove&&selected.some(s=>s.cat==="Small Cap")) continue;
      
      const w=Math.min(constraints.maxSingle, remaining, f.cat==="Index"?40:25);
      if(w<5) break;
      catMap[f.cat]=(catMap[f.cat]||0)+1;
      selected.push({...f,w});
      remaining-=w;
    }

    // Distribute leftover to index fund
    if(remaining>0&&selected.length>0){
      const idx=selected.findIndex(f=>f.cat==="Index");
      if(idx>=0) selected[idx].w+=remaining;
      else selected[0].w+=remaining;
    }

    // Normalize to 100
    const total=selected.reduce((s,f)=>s+f.w,0);
    selected.forEach(f=>f.w=Math.round(f.w/total*100));
    selected[0].w+=100-selected.reduce((s,f)=>s+f.w,0);

    const newScore=scoreAllocation(selected);
    const currentScore=holdings?scoreAllocation(holdings.map(h=>({...FUNDS[h.fund],w:Math.round(h.invested/(an?.totalInvested||1)*100)}))):{score:45,wER:1.2,wRet:12,wVol:18,wSh:0.7};

    await new Promise(r=>setTimeout(r,1200)); // simulate computation

    setResult({selected,newScore,currentScore,improvements:{
      cagr:  +(newScore.wRet-currentScore.wRet).toFixed(1),
      er:    +(currentScore.wER-newScore.wER).toFixed(2),
      vol:   +(currentScore.wVol-newScore.wVol).toFixed(1),
      sharpe:+(newScore.wSh-currentScore.wSh).toFixed(2),
    }});
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:350,
          messages:[{role:"user",content:`Explain in 3 sentences why this portfolio optimization improves the investor's position. Objective: ${objective}. Improvements: CAGR +${newScore.wRet-currentScore.wRet}%, ER savings -${(currentScore.wER-newScore.wER).toFixed(2)}%, volatility -${(currentScore.wVol-newScore.wVol).toFixed(1)}%. Top fund picks: ${selected.slice(0,3).map(f=>f.name).join(", ")}. Be specific about why each change helps the Indian retail investor.`}]})});
      const data=await res.json();
      setAiRationale(data.content?.[0]?.text||"");
    }catch{}
    setOptimizing(false);
  };

  const objButtons=[
    {k:"sharpe",l:"Max Sharpe Ratio",icon:"⚡",d:"Best risk-adjusted returns"},
    {k:"min_vol",l:"Min Volatility",icon:"🛡️",d:"Lowest portfolio risk"},
    {k:"max_return",l:"Max Returns",icon:"🚀",d:"Highest expected CAGR"},
  ];

  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    {/* Objective selector */}
    <Card>
      <SH title="Optimization Objective" icon="🎯" mb={12}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}} className="grid-3">
        {objButtons.map(o=><button key={o.k} onClick={()=>setObjective(o.k)} className="btn" style={{background:objective===o.k?T.gold+"18":T.raised,border:`1px solid ${objective===o.k?T.gold:T.faint}`,borderRadius:9,padding:"12px 14px",cursor:"pointer",textAlign:"left",color:T.ink}}>
          <div style={{fontSize:18,marginBottom:5}}>{o.icon}</div>
          <div style={{fontSize:11,fontWeight:700,color:objective===o.k?T.gold:T.ink}}>{o.l}</div>
          <div style={{fontSize:9,color:T.muted,marginTop:2}}>{o.d}</div>
        </button>)}
      </div>
    </Card>
    ...
    {/* Constraints */}
    <Card>
      <SH title="Constraints" icon="⚙️" mb={10}/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}} className="grid-3">
        {[
          {l:"Max Expense Ratio",k:"maxER",min:0.1,max:3,step:0.1,suffix:"%"},
          {l:"Max Single Fund",k:"maxSingle",min:10,max:60,step:5,suffix:"%"},
          {l:"Max Small Cap",k:"noSmallCapAbove",min:0,max:50,step:5,suffix:"%"},
        ].map(c=><div key={c.k}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
            <span style={{fontSize:10,color:T.muted}}>{c.l}</span>
            <Mono c={T.gold} s={{fontSize:11,fontWeight:700}}>{constraints[c.k]}{c.suffix}</Mono>
          </div>
          <input type="range" min={c.min} max={c.max} step={c.step} value={constraints[c.k]}
            onChange={e=>setConstraints(x=>({...x,[c.k]:+e.target.value}))} style={{width:"100%"}}/>
        </div>)}
      </div>
      <button onClick={runOptimizer} disabled={optimizing} className="btn" style={{marginTop:14,background:`linear-gradient(135deg,${T.gold},${T.teal})`,color:"#0B0F17",border:"none",borderRadius:8,padding:"11px 24px",fontWeight:800,fontSize:13,cursor:"pointer",width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
        {optimizing?<><Spin c="#0B0F17" sz={13}/>Running MPT Optimizer…</>:"🔬 Run Portfolio Optimizer"}
      </button>
    </Card>
    ...
    {result&&<>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}} className="grid-4">
        {[
          {l:"CAGR Change",v:`${result.improvements.cagr>0?"+":""}${result.improvements.cagr}%`,c:result.improvements.cagr>0?T.green:T.red},
          {l:"ER Savings",v:`-${result.improvements.er}%`,c:T.green},
          {l:"Volatility",v:`${result.improvements.vol>0?"-":""}${Math.abs(result.improvements.vol)}%`,c:result.improvements.vol>0?T.green:T.red},
          {l:"Sharpe Δ",v:`${result.improvements.sharpe>0?"+":""}${result.improvements.sharpe}`,c:result.improvements.sharpe>0?T.green:T.red},
        ].map(k=><Card key={k.l} style={{textAlign:"center",padding:"14px"}}>
          <div style={{fontSize:9,color:T.muted,marginBottom:5,textTransform:"uppercase"}}>{k.l}</div>
          <Mono c={k.c} s={{fontSize:20,fontWeight:900}}>{k.v}</Mono>
        </Card>)}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}} className="grid-2">
        <Card style={{border:`1px solid ${T.red}22`}}>
          <SH title="Current Portfolio" icon="📌" mb={10} right={<Badge color={T.muted}>Score: {result.currentScore.score}</Badge>}/>
          {holdings?holdings.map(h=>{
            const w=Math.round((h.invested/(an?.totalInvested||1))*100);
            return <div key={h.fund} style={{display:"flex",alignItems:"center",gap:9,marginBottom:7}}>
              <Mono c={T.red} s={{fontSize:12,fontWeight:700,width:35,flexShrink:0}}>{w}%</Mono>
              <div style={{flex:1}}>
                <div style={{fontSize:10,color:T.ink,marginBottom:2}}>{h.fund.split(" ").slice(0,3).join(" ")}</div>
                <Bar2 v={w} max={50} c={T.red}/>
              </div>
            </div>;
          }):<div style={{color:T.muted,fontSize:11}}>Import CAS for current allocation</div>}
        </Card>
        <Card style={{border:`1px solid ${T.green}22`}}>
          <SH title="Optimized Portfolio" icon="✨" mb={10} right={<Badge color={T.green}>Score: {result.newScore.score}</Badge>}/>
          {result.selected.map(f=><div key={f.name} style={{display:"flex",alignItems:"center",gap:9,marginBottom:7}}>
            <Mono c={T.green} s={{fontSize:12,fontWeight:700,width:35,flexShrink:0}}>{f.w}%</Mono>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                <span style={{fontSize:10,color:T.ink}}>{f.name.split(" ").slice(0,3).join(" ")}</span>
                <Badge color={T.ch[Object.keys(FUNDS).indexOf(f.name)%T.ch.length]}>{f.cat}</Badge>
              </div>
              <Bar2 v={f.w} max={50} c={T.green}/>
            </div>
            <div style={{textAlign:"right",flexShrink:0}}>
              <div style={{fontSize:9,color:T.muted}}>ER {f.er}%</div>
              <div style={{fontSize:9,color:T.teal}}>Sh {f.sharpe}</div>
            </div>
          </div>)}
        </Card>
      </div>
      ...
      {/* Optimizer chart */}
      <Card>
        <SH title="Allocation Breakdown" icon="🥧"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart><Pie data={result.selected.map(f=>({name:f.name.split(" ").slice(0,2).join(" "),value:f.w}))} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({name,value})=>`${value}%`}>
              {result.selected.map((_,i)=><Cell key={i} fill={T.ch[i%T.ch.length]}/>)}
            </Pie><Tooltip {...TT}/></PieChart>
          </ResponsiveContainer>
          <div style={{display:"flex",flexDirection:"column",gap:6,justifyContent:"center"}}>
            {result.selected.map((f,i)=><div key={f.name} style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{width:10,height:10,borderRadius:"50%",background:T.ch[i%T.ch.length],flexShrink:0}}/>
              <span style={{fontSize:10,color:T.body,flex:1}}>{f.name.split(" ").slice(0,3).join(" ")}</span>
              <Mono c={T.ch[i%T.ch.length]} s={{fontSize:11,fontWeight:700}}>{f.w}%</Mono>
            </div>)}
          </div>
        </div>
      </Card>
      ...
      {/* AI Rationale */}
      {aiRationale&&<Card style={{border:`1px solid ${T.violet}30`,background:T.violetDim}}>
        <SH title="AI Optimization Rationale" icon="🤖" mb={8}/>
        <p style={{fontSize:12,color:T.body,lineHeight:1.8,margin:0}}>{aiRationale}</p>
      </Card>}

      {/* Detailed comparison table */}
      <Card>
        <SH title="Optimized Fund Details" icon="📊" mb={10}/>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
            <thead><tr style={{borderBottom:`1px solid ${T.line}`}}>
              {["Fund","Category","Weight","1Y Ret","3Y Ret","ER","Sharpe","Vol","Beta"].map(h=><th key={h} style={{padding:"6px 10px",color:T.muted,fontWeight:600,textAlign:"left",fontSize:9,whiteSpace:"nowrap"}}>{h}</th>)}
            </tr></thead>
            <tbody>{result.selected.map((f,i)=><tr key={f.name} style={{borderBottom:`1px solid ${T.faint}`}}>
              <td style={{padding:"8px 10px",color:T.ink,fontWeight:600,maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{f.name}</td>
              <td style={{padding:"8px 10px"}}><Badge color={T.ch[i%T.ch.length]}>{f.cat}</Badge></td>
              <td style={{padding:"8px 10px"}}><Mono c={T.green} s={{fontWeight:700}}>{f.w}%</Mono></td>
              <td style={{padding:"8px 10px"}}><PnlChip v={f.r1||0}/></td>
              <td style={{padding:"8px 10px"}}><PnlChip v={f.r3||0}/></td>
              <td style={{padding:"8px 10px",color:T.body}}>{f.er}%</td>
              <td style={{padding:"8px 10px"}}><Mono c={T.teal} s={{fontSize:10}}>{f.sharpe}</Mono></td>
              <td style={{padding:"8px 10px",color:T.body}}>{f.vol}%</td>
              <td style={{padding:"8px 10px",color:T.body}}>{f.beta}</td>
            </tr>)}</tbody>
          </table>
        </div>
        <div style={{marginTop:12,fontSize:9,color:T.muted}}>⚠️ Optimized allocation is for analytical purposes only. Consult a SEBI-registered investment adviser before making changes. Past performance does not guarantee future results.</div>
      </Card>
    </>}
  </div>;
}
...
// ═══════════════════════════════════════════════════════════════
// FUND DISCOVERY ENGINE — Top funds by multiple metrics
// ═══════════════════════════════════════════════════════════════
function FundDiscoveryPage(){
  const [cat,setCat]=useState("all");
  const [sort,setSort]=useState("r3");
  const [search,setSearch]=useState("");
  const [compare,setCompare]=useState([]);
  const [aiRec,setAiRec]=useState("");
  const [aiLoading,setAiLoading]=useState(false);

  const cats=["all","Large Cap","Mid Cap","Small Cap","Flexi Cap","Index","Hybrid","Gold ETF","ELSS"];
  const sortOpts=[
    {k:"r3",l:"3Y Returns"},{k:"r1",l:"1Y Returns"},{k:"r5",l:"5Y Returns"},
    {k:"sharpe",l:"Sharpe Ratio"},{k:"er",l:"Expense Ratio ↓"},{k:"aum",l:"AUM"},
  ];

  const funds=Object.entries(FUNDS).map(([name,f])=>({name,...f}))
    .filter(f=>cat==="all"||f.cat===cat)
    .filter(f=>!search||f.name.toLowerCase().includes(search.toLowerCase())||f.amc.toLowerCase().includes(search.toLowerCase()))
    .sort((a,b)=>sort==="er"?(a[sort]||0)-(b[sort]||0):(b[sort]||0)-(a[sort]||0));

  // Top picks by category
  const topSharpe=Object.values(FUNDS).sort((a,b)=>(b.sharpe||0)-(a.sharpe||0)).slice(0,3);
  const topReturn=Object.values(FUNDS).sort((a,b)=>(b.r3||0)-(a.r3||0)).slice(0,3);
  const lowCost=Object.values(FUNDS).sort((a,b)=>(a.er||0)-(b.er||0)).slice(0,3);
  const topAlpha=Object.values(FUNDS).sort((a,b)=>(b.alpha||0)-(a.alpha||0)).slice(0,3);

  const getAIRec=async()=>{
    setAiLoading(true);
    try{
      const topFunds=funds.slice(0,5).map(f=>`${f.name} (${f.cat}, ${f.r3}% 3Y, ER ${f.er}%)`).join("; ");
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:300,
          messages:[{role:"user",content:`Based on these top Indian MF results filtered by ${cat} category sorted by ${sort}: ${topFunds}. Give a 2-sentence insight about what stands out and who these funds are best suited for. Be specific about Indian market conditions.`}]})});
      const data=await res.json();
  ...
  const inp={background:T.raised,border:`1px solid ${T.line}`,color:T.ink,borderRadius:7,padding:"8px 12px",fontSize:12,outline:"none"};

  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    {/* Top picks row */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}} className="grid-4">
      {[{l:"Highest Sharpe",funds:topSharpe,c:T.gold,icon:"⭐"},{l:"Best 3Y Return",funds:topReturn,c:T.green,icon:"📈"},{l:"Lowest Cost",funds:lowCost,c:T.teal,icon:"💸"},{l:"Highest Alpha",funds:topAlpha,c:T.violet,icon:"🔬"}].map(({l,funds:fds,c,icon})=><Card key={l} style={{border:`1px solid ${c}22`}}>
        <div style={{fontSize:10,color:c,fontWeight:700,marginBottom:8}}>{icon} {l}</div>
        {fds.map((f,i)=>{
          const name=Object.keys(FUNDS).find(n=>FUNDS[n]===f)||"";
          return <div key={i} style={{padding:"5px 0",borderBottom:i<2?`1px solid ${T.faint}`:"none"}}>
            <div style={{fontSize:9,color:T.ink,fontWeight:600,marginBottom:1}}>{name.split(" ").slice(0,3).join(" ")}</div>
            <div style={{fontSize:9,color:c,fontFamily:"'Space Mono',monospace"}}>
              {l==="Highest Sharpe"?f.sharpe:l==="Best 3Y Return"?`${f.r3}%`:l==="Lowest Cost"?`${f.er}%`:`+${f.alpha}%`}
            </div>
          </div>;
        })}
      </Card>)}
    ...
    <Card style={{padding:"12px 14px"}}>
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search funds or AMC…" style={{...inp,width:200}}/>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {cats.map(c=><button key={c} onClick={()=>setCat(c)} className="btn" style={{background:cat===c?T.gold+"22":"transparent",border:`1px solid ${cat===c?T.gold:T.line}`,color:cat===c?T.gold:T.muted,borderRadius:5,padding:"4px 9px",fontSize:9,cursor:"pointer",fontWeight:cat===c?700:400}}>{c}</button>)}
        </div>
        <select value={sort} onChange={e=>setSort(e.target.value)} style={{...inp,cursor:"pointer"}}>
          {sortOpts.map(o=><option key={o.k} value={o.k}>{o.l}</option>)}
        </select>
        <button onClick={getAIRec} disabled={aiLoading} className="btn" style={{background:`linear-gradient(135deg,${T.violet},${T.blue})`,color:"#fff",border:"none",borderRadius:6,padding:"7px 12px",fontSize:10,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:5,marginLeft:"auto"}}>
          {aiLoading?<><Spin c="#fff" sz={10}/>…</>:"🤖 AI Pick"}
        </button>
      </div>
      {aiRec&&<div style={{marginTop:10,padding:"9px 12px",background:T.violetDim,borderRadius:7,border:`1px solid ${T.violet}30`,fontSize:11,color:T.body,lineHeight:1.7}}>{aiRec}</div>}
    </Card>

    {/* Fund cards grid */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}} className="grid-3">
      {funds.map(f=>{
        const inCompare=compare.includes(f.name);
        return <div key={f.name} style={{background:T.card,border:`1px solid ${inCompare?T.gold:T.faint}`,borderRadius:10,padding:"13px",transition:"border .15s"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:11,fontWeight:700,color:T.ink,marginBottom:3,lineHeight:1.3}}>{f.name}</div>
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                <Badge color={T.teal}>{f.cat}</Badge>
                <Badge color={T.muted}>{f.amc.split(" ")[0]}</Badge>
              </div>
            </div>
            <button onClick={()=>setCompare(c=>inCompare?c.filter(x=>x!==f.name):[...c.slice(-2),f.name])} style={{background:inCompare?T.gold+"22":T.raised,border:`1px solid ${inCompare?T.gold:T.faint}`,color:inCompare?T.gold:T.muted,borderRadius:5,padding:"3px 7px",fontSize:9,cursor:"pointer",flexShrink:0}}>
              {inCompare?"✓ Added":"+ Compare"}
            </button>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,marginBottom:9}}>
            {[{l:"1Y",v:`${f.r1||0}%`,c:T.green},{l:"3Y",v:`${f.r3||0}%`,c:T.teal},{l:"5Y",v:`${f.r5||0}%`,c:T.blue}].map(m=><div key={m.l} style={{background:T.raised,borderRadius:6,padding:"5px",textAlign:"center"}}>
              <div style={{fontSize:8,color:T.muted}}>{m.l}</div>
              <Mono c={m.c} s={{fontSize:11,fontWeight:700}}>{m.v}</Mono>
            </div>)}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:T.muted}}>
            <span>ER <Mono c={T.amber} s={{fontSize:9}}>{f.er}%</Mono></span>
            <span>Sharpe <Mono c={T.teal} s={{fontSize:9}}>{f.sharpe}</Mono></span>
            <span>AUM ₹<Mono c={T.body} s={{fontSize:9}}>{(f.aum/100).toFixed(0)}K Cr</Mono></span>
            <span>NAV <Mono c={T.ink} s={{fontSize:9}}>₹{f.nav}</Mono></span>
          </div>
          <div style={{marginTop:8}}>
            <div style={{height:2,background:T.faint,borderRadius:2}}>
              <div style={{height:"100%",width:`${Math.min(100,(f.r3||0)*3)}%`,background:`linear-gradient(90deg,${T.teal},${T.green})`,borderRadius:2}}/>
            </div>
          </div>
        </div>;
      })}
    </div>
    {funds.length===0&&<div style={{color:T.muted,textAlign:"center",padding:"40px",fontSize:12}}>No funds match your filters.</div>}
    <div style={{fontSize:10,color:T.muted}}>Showing {funds.length} funds · Data sourced from AMFI · Returns annualised</div>
  </div>;
}

// ═══════════════════════════════════════════════════════════════
// ADVISOR DASHBOARD — B2B Multi-Client Portfolio Management
// ═══════════════════════════════════════════════════════════════
function AdvisorDashboardPage(){
  const CLIENTS=[
    {id:1,name:"Rajesh Mehta",pan:"ABCPM1234R",risk:"Moderate",plan:"Pro",value:4280000,invested:3100000,xirr:18.4,health:78,funds:5,lastReview:"2024-10-15",alerts:2,email:"rajesh@example.com"},
    {id:2,name:"Priya Sharma",pan:"XYZPS5678T",risk:"Aggressive",plan:"Pro",value:1890000,invested:1500000,xirr:22.1,health:65,funds:7,lastReview:"2024-10-08",alerts:4,email:"priya@example.com"},
    {id:3,name:"Suresh Agarwal",pan:"PQRSA9012U",risk:"Conservative",plan:"Free",value:920000,invested:900000,xirr:7.8,health:82,funds:3,lastReview:"2024-09-20",alerts:0,email:"suresh@example.com"},
    {id:4,name:"Kavita Patel",pan:"LMNKP3456V",risk:"Moderate",plan:"Pro",value:6750000,invested:4200000,xirr:24.6,health:71,funds:8,lastReview:"2024-10-12",alerts:1,email:"kavita@example.com"},
    {id:5,name:"Amit Joshi",pan:"DEFAJ7890W",risk:"Aggressive",plan:"Pro",value:2340000,invested:1800000,xirr:15.2,health:58,funds:6,lastReview:"2024-09-30",alerts:3,email:"amit@example.com"},
  ];
  ...
  const totalAUM=CLIENTS.reduce((s,c)=>s+c.value,0);
  const avgHealth=Math.round(CLIENTS.reduce((s,c)=>s+c.health,0)/CLIENTS.length);
  const totalAlerts=CLIENTS.reduce((s,c)=>s+c.alerts,0);
    ...
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:400,
          messages:[{role:"user",content:`As a SEBI-registered investment adviser, write a 3-sentence portfolio review for client ${client.name}. Portfolio: ₹${(client.value/1e5).toFixed(1)}L value, XIRR ${client.xirr}%, health score ${client.health}/100, risk profile: ${client.risk}, ${client.funds} funds. Cover: performance assessment, key concern, one actionable recommendation. Use professional but warm tone.`}]})});
      const data=await res.json();
      setClientReport(data.content?.[0]?.text||"");
    }catch{}
    setGenerating(false);
  };

  const riskColor={Conservative:T.blue,Moderate:T.teal,Aggressive:T.red};
  const inp={background:T.raised,border:`1px solid ${T.line}`,color:T.ink,borderRadius:7,padding:"7px 11px",fontSize:11,outline:"none"};

  return <div style={{display:"flex",gap:14,height:"calc(100vh - 160px)",minHeight:600}}>
    {/* Client list sidebar */}
    <div style={{width:280,flexShrink:0,display:"flex",flexDirection:"column",gap:8}}>
      <Card style={{padding:"10px 12px"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search clients…" style={{...inp,width:"100%"}}/>
      </Card>
      <div style={{display:"flex",flexDirection:"column",gap:6,overflowY:"auto",flex:1}}>
        {filtered.map(c=>{
          const pnl=c.value-c.invested;const pnlP=pnl/c.invested*100;
          return <div key={c.id} onClick={()=>{setSelected(c);setClientReport("");setTab("overview");}} style={{background:selected?.id===c.id?T.hover:T.card,border:`1px solid ${selected?.id===c.id?T.gold:T.faint}`,borderRadius:8,padding:"10px 12px",cursor:"pointer",transition:"all .12s"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:5}}>
              <div>
                <div style={{fontSize:11,fontWeight:700,color:T.ink}}>{c.name}</div>
                <Badge color={riskColor[c.risk]}>{c.risk}</Badge>
              </div>
              <div style={{textAlign:"right"}}>
                {c.alerts>0&&<div style={{background:T.red,color:"#fff",borderRadius:10,fontSize:8,fontWeight:700,padding:"1px 5px",marginBottom:3}}>{c.alerts} alerts</div>}
                <Badge color={c.plan==="Pro"?T.gold:T.muted}>{c.plan}</Badge>
              </div>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <Mono c={T.ink} s={{fontSize:11,fontWeight:700}}>₹{(c.value/1e5).toFixed(1)}L</Mono>
              <PnlChip v={+pnlP.toFixed(1)}/>
            </div>
            <div style={{marginTop:5}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:2,fontSize:8,color:T.muted}}>
                <span>Health</span><span>{c.health}/100</span>
              </div>
              <div style={{background:T.faint,borderRadius:3,height:3}}>
                <div style={{width:`${c.health}%`,height:"100%",background:c.health>=70?T.green:c.health>=50?T.amber:T.red,borderRadius:3}}/>
              </div>
            </div>
          </div>;
        })}
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",gap:10,overflowY:"auto"}}>
      {!selected?<>
        {/* AUM Overview */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}} className="grid-4">
          <Card><KPI label="Total AUM" value={`₹${(totalAUM/1e7).toFixed(2)}Cr`} color={T.gold} lg/></Card>
          <Card><KPI label="Active Clients" value={CLIENTS.length} color={T.teal}/></Card>
          <Card><KPI label="Avg Health Score" value={`${avgHealth}/100`} color={avgHealth>=70?T.green:T.amber}/></Card>
          <Card><KPI label="Total Alerts" value={totalAlerts} color={totalAlerts>0?T.red:T.green}/></Card>
        </div>

        {/* AUM distribution */}
        <Card>
          <SH title="Client AUM Distribution" icon="📊" mb={10}/>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={CLIENTS.map(c=>({name:c.name.split(" ")[0],value:Math.round(c.value/1e5),invested:Math.round(c.invested/1e5)}))}>
              <CartesianGrid strokeDasharray="3 3" stroke={T.faint}/>
              <XAxis dataKey="name" tick={{fill:T.muted,fontSize:10}}/>
              <YAxis tick={{fill:T.muted,fontSize:9}} tickFormatter={v=>`₹${v}L`}/>
              <Tooltip {...TT} formatter={v=>`₹${v}L`}/>
              <Legend wrapperStyle={{fontSize:10}}/>
              <Bar name="Current Value" dataKey="value" fill={T.gold} radius={[3,3,0,0]}/>
              <Bar name="Invested" dataKey="invested" fill={T.teal} radius={[3,3,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </Card>

        {/* Alert summary */}
        {totalAlerts>0&&<Card style={{border:`1px solid ${T.red}28`,background:T.redDim}}>
          <SH title="Clients Needing Attention" icon="🚨" mb={10}/>
          {CLIENTS.filter(c=>c.alerts>0).map(c=><div key={c.id} onClick={()=>setSelected(c)} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:6,cursor:"pointer",marginBottom:5,background:T.card}} className="hov">
            <div style={{flex:1}}>
              <span style={{fontSize:11,fontWeight:600,color:T.ink}}>{c.name}</span>
              <span style={{fontSize:10,color:T.muted,marginLeft:8}}>₹{(c.value/1e5).toFixed(1)}L · Health {c.health}</span>
            </div>
            <Badge color={T.red}>{c.alerts} alerts</Badge>
          </div>)}
        </Card>}
      </>:<>
        {/* Client detail header */}
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
          <button onClick={()=>setSelected(null)} style={{background:T.raised,border:`1px solid ${T.line}`,color:T.muted,borderRadius:6,padding:"5px 10px",cursor:"pointer",fontSize:10}}>← All Clients</button>
          <div style={{flex:1}}>
            <div style={{fontSize:14,fontWeight:800,color:T.ink}}>{selected.name}</div>
            <div style={{fontSize:10,color:T.muted}}>PAN: {selected.pan} · {selected.email}</div>
          </div>
          <Badge color={riskColor[selected.risk]}>{selected.risk}</Badge>
          {selected.alerts>0&&<Badge color={T.red}>{selected.alerts} alerts</Badge>}
        </div>

        {/* Tabs */}
        <div style={{display:"flex",gap:0,borderBottom:`1px solid ${T.line}`,marginBottom:2}}>
          {["overview","analytics","report"].map(t=><button key={t} onClick={()=>setTab(t)} style={{background:"none",border:"none",borderBottom:tab===t?`2px solid ${T.gold}`:"2px solid transparent",color:tab===t?T.gold:T.muted,padding:"7px 14px",cursor:"pointer",fontSize:11,fontWeight:tab===t?700:400,fontFamily:"inherit",textTransform:"capitalize"}}>{t}</button>)}
        </div>

        {tab==="overview"&&<>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}} className="grid-4">
            <Card><KPI label="Portfolio Value" value={`₹${(selected.value/1e5).toFixed(1)}L`} color={T.gold} lg/></Card>
            <Card><KPI label="XIRR" value={`${selected.xirr}%`} color={T.green}/></Card>
            <Card><KPI label="Health Score" value={`${selected.health}/100`} color={selected.health>=70?T.green:selected.health>=50?T.amber:T.red}/></Card>
            <Card><KPI label="No. of Funds" value={selected.funds} color={T.teal}/></Card>
          </div>
          <Card>
            <SH title="Last Review" icon="📅" mb={8}/>
            <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
              <div style={{fontSize:11,color:T.body}}>Last reviewed: <strong style={{color:T.ink}}>{new Date(selected.lastReview).toLocaleDateString("en-IN",{dateStyle:"long"})}</strong></div>
              <div style={{fontSize:11,color:T.body}}>Days since review: <strong style={{color:T.amber}}>{Math.round((new Date()-new Date(selected.lastReview))/864e5)} days</strong></div>
              <div style={{fontSize:11,color:T.body}}>Plan: <Badge color={selected.plan==="Pro"?T.gold:T.muted}>{selected.plan}</Badge></div>
            </div>
          </Card>
        </>}

        {tab==="analytics"&&<Card>
          <SH title="Portfolio Analytics" icon="📈" mb={12}/>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}} className="grid-3">
            {[
              {l:"Total Invested",v:`₹${(selected.invested/1e5).toFixed(1)}L`,c:T.body},
              {l:"Current Value",v:`₹${(selected.value/1e5).toFixed(1)}L`,c:T.ink},
              {l:"P&L",v:`₹${((selected.value-selected.invested)/1e5).toFixed(1)}L`,c:selected.value>selected.invested?T.green:T.red},
              {l:"XIRR",v:`${selected.xirr}%`,c:selected.xirr>12?T.green:T.amber},
              {l:"Risk Profile",v:selected.risk,c:riskColor[selected.risk]},
              {l:"Health Score",v:`${selected.health}/100`,c:selected.health>=70?T.green:T.amber},
            ].map(m=><div key={m.l} style={{background:T.raised,borderRadius:7,padding:"10px 12px"}}>
              <div style={{fontSize:9,color:T.muted,marginBottom:4,textTransform:"uppercase"}}>{m.l}</div>
              <Mono c={m.c} s={{fontSize:14,fontWeight:700}}>{m.v}</Mono>
            </div>)}
          </div>
        </Card>}

        {tab==="report"&&<Card>
          <SH title="Client Report Generator" icon="📑" mb={12}/>
          {!clientReport&&!generating&&<div style={{textAlign:"center",padding:"20px 0"}}>
            <button onClick={()=>generateReport(selected)} className="btn" style={{background:`linear-gradient(135deg,${T.gold},${T.teal})`,color:"#0B0F17",border:"none",borderRadius:8,padding:"11px 24px",fontWeight:700,fontSize:12,cursor:"pointer"}}>
              🤖 Generate AI Review for {selected.name.split(" ")[0]}
            </button>
          </div>}
          {generating&&<div style={{display:"flex",alignItems:"center",gap:8,color:T.muted,fontSize:12,padding:"20px 0"}}><Spin/>Generating personalised review…</div>}
          {clientReport&&<>
            <div style={{background:T.violetDim,border:`1px solid ${T.violet}30`,borderRadius:8,padding:"14px",marginBottom:12}}>
              <p style={{fontSize:12,color:T.body,lineHeight:1.8,margin:0}}>{clientReport}</p>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>window.print()} className="btn" style={{background:T.gold,color:"#0B0F17",border:"none",borderRadius:6,padding:"8px 16px",fontWeight:700,fontSize:11,cursor:"pointer"}}>🖨️ Print Report</button>
              <button onClick={()=>generateReport(selected)} className="btn" style={{background:T.raised,border:`1px solid ${T.line}`,color:T.ink,borderRadius:6,padding:"8px 14px",fontSize:11,cursor:"pointer"}}>↻ Regenerate</button>
            </div>
          </>}
        </Card>}
      </>}
...
const PAGES=[
  ...
  {id:"optimizer",  icon:"🔬",label:"Optimizer",    group:"quant"},
  {id:"goals",     icon:"🏅",label:"Goal Tracker",  group:"quant"},
  {id:"ai",        icon:"🤖",label:"AI Advisor",    group:"intelligence"},
  {id:"discovery",  icon:"🔭",label:"Fund Discovery",group:"intelligence"},
  {id:"advisor",    icon:"👔",label:"Advisor Mode",  group:"intelligence"},
...
export default function App(){
  ...
  const pageTitles={
    optimizer:"Portfolio Optimizer",discovery:"Fund Discovery Engine",advisor:"Advisor Dashboard",
  ...
  return <div style={{minHeight:"100vh",background:bgColor,color:inkColor,fontFamily:"'Space Grotesk',sans-serif"}}>
    ...
    <main style={{maxWidth:1600,margin:"0 auto",padding:"18px 16px 40px"}}>
      ...
      <div className="fu" key={page}>
        ...
        {page==="optimizer"  &&<PortfolioOptimizerPage an={an} holdings={holdings}/>}
        {page==="discovery"  &&<FundDiscoveryPage/>}
        {page==="advisor"    &&<AdvisorDashboardPage/>}
